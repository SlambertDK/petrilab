...
### 4.4 From per-knob credit to statistical inference

The Gardener's original memory was a decayed running mean (EWMA) of each knob's effect — enough to bias selection, but unable to say whether an effect is *real* or merely noise around zero. Because the observed complexity peaks oscillate (Section 5), an average slightly above zero after thousands of trials could equally be a weak true signal or drift. The system therefore now carries an explicit inference layer that answers the question a researcher would ask first: *does this condition correlate with success, or did the success just happen?* [gardener.py, analytics.py, stats_core.py]

The first version of this layer used a naïve Pearson-correlation *t*-test with the raw sample size, which is exactly the wrong tool here and was corrected after adversarial review. The observations are not i.i.d.: they are a single autocorrelated trajectory, the interventions are chosen by a policy that conditions on outcomes, and the project's own key finding (H0008) is that the interesting structure lives in *interactions*, not single knobs — every one of which biases a naïve test toward *over*-stating confidence. The hardened layer addresses each:

**(i) A reproducible observation log.** Every resolved experiment — kept *and* reverted — is appended as one row recording the condition changed, its before/after score delta, the full condition vector, and the outcome variables (MODES axes, cell and lineage counts, largest genome) [gardener.py]. Logging reverted experiments too is deliberate: analysing only retained states would condition on the outcome and manufacture associations (selection/collider bias).

**(ii) Autocorrelation-corrected, family-wise-controlled correlation.** For each condition×outcome pair the layer computes Pearson *r*, cross-checks it against Spearman rank correlation, and tests significance with a *t*-test that uses the AR(1) **effective sample size** $n_\text{eff} = n\,(1-\rho)/(1+\rho)$ rather than $n$, so serial dependence cannot inflate significance [stats_core.py]. In practice this is decisive: on live data the complexity series carries lag-1 autocorrelation $\rho \approx 0.84$, collapsing $n \approx 280$ logged experiments to $n_\text{eff} \approx 24$ independent ones. All $7\times 6 = 42$ tests are then corrected together with the **Benjamini–Hochberg** false-discovery-rate procedure at $\alpha = 0.10$ [B&H 1995; stats_core.py], and a verdict of "significant" additionally requires Pearson and Spearman to agree in sign. A correlation that survives naïvely but dies under $n_\text{eff}$ + FDR was never real.

**(iii) A causal layer that exploits the interventions.** Because the Gardener performs genuine before/after manipulations, per-knob score deltas are analysed as a paired sample with the **Wilcoxon signed-rank** and sign tests (nonparametric, using direction and magnitude), reported with **Cohen's $d_z$** effect size and FDR correction [stats_core.py]. This is stronger evidence than log-wide correlation, which is confounded by everything the agent does simultaneously.

**(iv) Robust trend detection.** The class-2/class-4 question — are complexity peaks *climbing* (accumulation) or merely *repeating* (bounded oscillation)? — is tested with the **Mann–Kendall** trend test and **Sen's slope**, deflated for autocorrelation, replacing an earlier least-squares fit to the peak envelope. Fitting a line to selected upper-envelope maxima is itself a bias: it yields a non-negative apparent slope even for pure stationary oscillation, and was removed for that reason.

**(v) An interaction screen for the univariate blind spot.** Because the project's own key finding (H0008) locates the signal in *combinations* of conditions rather than single knobs, a purely one-variable-at-a-time analysis is structurally unable to see the very effect the project cares about most. Each knob *pair* is therefore screened for a two-way interaction on complexity, controlling for both main effects, using a **moving-block Freedman–Lane permutation test** [36] that preserves the run's autocorrelation under the null (a plain residual shuffle would assume exchangeability and manufacture interactions), FDR-corrected across all pairs. This screen is deliberately framed as *hypothesis-generating, not confirmatory*: because the Gardener changes one knob at a time, the predictors are themselves autocorrelated, which block-permutation only partly undoes, so any flagged pair is reported as a **candidate for a controlled A/B follow-up** — exactly the frozen-rules, one-mechanism-at-a-time discipline of Section 3.1 — never as a confirmed interaction. It restores what the univariate layer cannot see while refusing to overclaim on what it finds.

These models are descriptive, dependency-free, and every number traces back to the log; they are rendered on demand as a visual report with an explicit *threats to validity* note [analytics.py]. Crucially, the honest current outcome is a **null**: after autocorrelation and multiplicity correction, *no* single condition shows a significant, sign-stable correlation with complexity, and Mann–Kendall finds no monotone accumulation trend. The naïve first-pass test would have declared structural_heredity "significant"; the corrected test shows that verdict was an artifact of treating 24 effective observations as 280. Reporting the weaker, correct answer — rather than the stronger, wrong one — is the point of the layer. The interaction screen (v) does surface several knob-*pair* candidates that the univariate nulls miss, but these are held as hypotheses for controlled follow-up, not banked as findings.
...
Longer runs exposed the failure modes the anti-homeostasis mechanisms were built to answer. The v1 novelty metric decayed to zero after roughly 500k generations for a purely mechanical reason (an unbounded "seen" set), which is why v2 rebuilt it with a sliding window [modes.py]. In long v2 runs the dish tends to drift toward **monoculture** — living lineages fall away until the population is dominated by one line — which is why the immigration guard exists and fires when the lineage count drops below its floor [petrilab.py]. When the complexity proxy flatlines the reheating mechanism triggers, temporarily raising mutation before cooling back down [petrilab.py]; the Gardener counts these reheating events and reports them in its conclusions as evidence that anti-stagnation machinery is being exercised [gardener.py]. The overall picture is not "sustained open-endedness achieved" but "a system that repeatedly approaches homeostasis and has explicit, measured mechanisms that push it back off the fixed point — with the open question of whether that push produces class-4 accumulation or merely a higher-order class-2 cycle". The inference layer (Section 4.4) now answers that question on the data collected so far, and the answer is sobering — and more sobering after the analysis was hardened. The Mann–Kendall trend test on live complexity finds no monotone accumulation (peaks repeat rather than climb), and once the correlation tests are corrected for autocorrelation ($n \approx 280$ experiments amount to only $n_\text{eff} \approx 24$ independent ones at lag-1 $\rho \approx 0.84$) and for multiple comparisons, no single condition shows a statistically significant, sign-stable correlation with the large-cell events — and the paired-intervention causal test likewise flags no knob as reliably raising the score. A naïve first-pass test *did* flag structural_heredity as significant; the correction revealed that as an artifact of over-counting dependent observations, and it was removed. The giant, all-green cells are real and striking to watch, but on the evidence to date they are recurrences of the same high notes, not an accumulating staircase — precisely the class-2 outcome the contract is designed to refuse to call a success.
...
[32] Benjamini, Y., & Hochberg, Y. (1995). Controlling the False Discovery Rate: A Practical and Powerful Approach to Multiple Testing. *Journal of the Royal Statistical Society, Series B*, 57(1), 289–300. (FDR control across the condition×outcome test family, Section 4.4.)

[33] Wilcoxon, F. (1945). Individual Comparisons by Ranking Methods. *Biometrics Bulletin*, 1(6), 80–83. (Paired signed-rank test on the intervention deltas, Section 4.4.)

[34] Mann, H. B. (1945). Nonparametric Tests Against Trend. *Econometrica*, 13(3), 245–259; and Kendall, M. G. (1975). *Rank Correlation Methods*, 4th ed., Charles Griffin. (Mann–Kendall monotone-trend test for the accumulation-vs-oscillation question.)

[35] Sen, P. K. (1968). Estimates of the Regression Coefficient Based on Kendall's Tau. *Journal of the American Statistical Association*, 63(324), 1379–1389. (Robust trend-magnitude estimator paired with Mann–Kendall.)

[36] Freedman, D., & Lane, D. (1983). A Nonstochastic Interpretation of Reported Significance Levels. *Journal of Business & Economic Statistics*, 1(4), 292–298; permutation of reduced-model residuals extended to a moving-block scheme after Künsch, H. R. (1989). The Jackknife and the Bootstrap for General Stationary Observations. *Annals of Statistics*, 17(3), 1217–1241. (Autocorrelation-preserving interaction screen, Section 4.4(v).)

[37] Bayley, G. V., & Hammersley, J. M. (1946). The "Effective" Number of Independent Observations in an Autocorrelated Time Series. *Supplement to the JRSS*, 8(2), 184–197. (AR(1) effective-sample-size correction $n_\text{eff}=n(1-\rho)/(1+\rho)$, Section 4.4.)

[38] Wasserstein, R. L., & Lazar, N. A. (2016). The ASA Statement on p-Values: Context, Process, and Purpose. *The American Statistician*, 70(2), 129–133. (Effect sizes and confidence over bare p-values, Section 4.4.)
