...
The Gardener's original memory was a decayed running mean (EWMA) of each knob's effect — enough to bias selection, but unable to say whether an effect is *real* or merely noise around zero. Because the observed complexity peaks oscillate (Section 5), an average slightly above zero after thousands of trials could equally be a weak true signal or drift. The system therefore now carries an explicit inference layer that answers the question a researcher would ask first: *does this condition correlate with success, or did the success just happen?* [gardener.py, analytics.py]

Three additions implement this. **(i)** Every resolved experiment is appended to an observation log — one row recording the condition changed, the full condition vector at decision time, and the outcome variables (the MODES axes, cell and lineage counts, largest genome) [gardener.py]. This is a reproducible dataset, not a summary statistic. **(ii)** Over that log the system computes a Pearson correlation between each condition and each outcome, and tests each correlation for significance with a one-sample *t*-test, `t = r·√((n−2)/(1−r²))`, reporting a verdict at the conventional thresholds |t| ≥ 1.96 (p < 0.05) and |t| ≥ 2.58 (p < 0.01) [analytics.py]. A per-knob effect is additionally tested against the null of zero effect using an online Welford variance, so "significant" and "chance" are distinguished rather than asserted [gardener.py]. **(iii)** A complexity-trend estimator fits a least-squares line to the upper envelope (top-quartile peaks) of the recent complexity history and reports its slope and correlation, operationalising the paper's central open question as a measurement: growing peaks indicate class-4 accumulation, a flat envelope indicates class-2 oscillation [gardener.py].

These models are descriptive and fully reproducible — no external libraries, every number traceable to the log — and are rendered on demand as a visual report (correlation tables with significance verdicts, a diverging-bar "recipe" of which conditions co-move with larger cells, and univariate regressions with R²) [analytics.py]. The layer does not license causal claims: it detects and grades *correlation*, and it is explicit that low single-knob R² alongside no significant correlation is itself an honest null result — evidence that the large-cell events are, so far, oscillation rather than a controllable recipe [analytics.py]. This is exactly the class-2/class-4 discrimination the falsification contract (Section 4.2) demands, now supplied with a statistical test rather than a visual impression.
